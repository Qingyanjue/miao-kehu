...
import { recordPlayHistory } from '@/api/system'
...
// 加载当前歌曲
  const loadTrack = async () => {
    ...
    if (audioElement.value) {
      ...
      // 加锁：只有在用户已登录，且歌曲 ID 存在时，才发送请求
      if (userStore.isLoggedIn && currentTrack.value && currentTrack.value.id) {
        recordPlayHistory(currentTrack.value.id).catch(err => {
          console.log('静默记录播放历史失败:', err)
        })
      }
    }
  }
